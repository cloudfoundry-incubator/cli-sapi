...
var _ = Describe("Service Instance Actions", func() {
	...
	Describe("GetServiceInstanceDetails", func() {
		...
		When("the cloud controller request is successful", func() {
			...
			It("returns the correct service instance details object", func() {
				Expect(serviceInstance).To(Equal(
					ServiceInstanceDetails{
						...
						UpgradeStatus:           UpgradeNotSupported,
					},
				))
			})
			...
			Describe("UpgradeStatus", func() {
				When("the instance says an upgrade is available", func() {
					BeforeEach(func() {
						fakeCloudControllerClient.GetServiceInstanceByNameAndSpaceReturns(
							resources.ServiceInstance{
								Type:             resources.ManagedServiceInstance,
					...
					It("says an upgrade is available", func() {
						Expect(serviceInstance.UpgradeStatus).To(Equal(UpgradeAvailable))
					})
				})

				When("the instance has MI and says an upgrade is not available", func() {
					BeforeEach(func() {
						fakeCloudControllerClient.GetServiceInstanceByNameAndSpaceReturns(
							resources.ServiceInstance{
								Type:                   resources.ManagedServiceInstance,
					...
					It("says an upgrade is not available", func() {
						Expect(serviceInstance.UpgradeStatus).To(Equal(UpgradeNotAvailable))
					})
				})

				When("the instance has no MI and says an upgrade is not available", func() {
					BeforeEach(func() {
						fakeCloudControllerClient.GetServiceInstanceByNameAndSpaceReturns(
							resources.ServiceInstance{
								Type:                   resources.ManagedServiceInstance,
					...
					It("says an upgrade is not supported", func() {
						Expect(serviceInstance.UpgradeStatus).To(Equal(UpgradeNotSupported))
					})
				})
			})
		})
